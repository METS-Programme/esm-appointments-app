    ...
    case "CheckedIn":
      return (
        <Button
          ...
          onClick={() => handleComplete(appointmentUuid, mutate, t)}
        >
          {t("complete", "Complete")}
        </Button>
      );
...
const AppointmentsBaseTable = () => {
  ...
  const fullView =
    userHasAccess(fullViewPrivilege, user) || !useFullViewPrivilege;
  ...
  const tableRows = paginatedAppointments?.map((appointment) => ({
    ...
    location: {
      content: (
        <span className={styles.statusContainer}>{appointment.location}</span>
      ),
