...
import {
  useInitialAppointmentFormValue,
  type PatientAppointment,
} from "../useInitialFormValues";
...
const AppointmentForm: React.FC<AppointmentFormProps> = ({
  ...
  const [patientAppointment, setPatientAppointment] =
    useState<PatientAppointment>(initialAppointmentFormValues);
          ...
          <Row className={styles.row}>
            ...
            <Column sm={3}>
              <Select
                ...
                onChange={(event) =>
                  setPatientAppointment({
                    ...patientAppointment,
                    serviceUuid: event.target.value,
                  })
                }
              ...
              >
                ...
                patientAppointment.serviceUuid == "--" ? (
                  <SelectItem
                    text={t("chooseService", "Select service")}
                    value=""
                  />
                ) : null}
                {services?.length > 0 &&
            ...
            <Column sm={3}>
              <Select
                ...
                onChange={(event) =>
                  setPatientAppointment({
                    ...patientAppointment,
                    appointmentKind: event.target.value,
                  })
                }
                value={patientAppointment.appointmentKind}
              >
                {!patientAppointment.appointmentKind ||
                patientAppointment.appointmentKind == "--" ? (
                  <SelectItem
                    text={t(
                      "selectAppointmentType",
            ...
            <Column sm={3}>
              <Select
                ...
                onChange={(event) => {
                  setPatientAppointment({
                    ...patientAppointment,
                    providerUuid: event.target.value,
                  });
                }}
              ...
              >
                {!patientAppointment.providerUuid ? (
                  <SelectItem
                    text={t("chooseProvider", "Select Provider")}
                    value=""
                  />
                ) : null}
                {providers?.length > 0 &&
