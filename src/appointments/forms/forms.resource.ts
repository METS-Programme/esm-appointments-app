...
export function useProviders() {
  const { data, error, isLoading } = useSWR<
    ...
    Error
  >(`/ws/rest/v1/provider`, openmrsFetch);
...
export const useAppointmentSummary = (
  ...
  const { data, error } = useSWR<{ data: Array<AppointmentSummary> }>(
    url,
    openmrsFetch,
  );
  const results = first(
    data?.data.filter(
      ({ appointmentService }) => appointmentService.uuid === serviceUuid,
    ),
  );
  ...
  return Object.entries(appointmentCountMap ?? [])
    ...
    .sort(
      (dateA, dateB) =>
        new Date(dateA.date).getTime() - new Date(dateB.date).getTime(),
    );
};
...
export const toAppointmentDateTime = (
  fromDate: Date,
  hours: number,
  minutes: number,
) => {
