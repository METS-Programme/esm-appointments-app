...
const ScheduledAppointments: React.FC<ScheduledAppointmentsProps> = ({
  ...
  useEffect(() => {
    ...
    if (
      allowedExtensions &&
      (currentTab === null || !allowedExtensions[currentTab])
    ) {
  ...
  return (
    <>
      <ContentSwitcher
        ...
        selectedIndex={
          panelsToShow.findIndex((panel) => panel.name == currentTab) ?? 0
        }
...
function useAllowedExtensions() {
  ...
  return {
    ...
    showExtension: (extension: string) =>
      dispatch({ type: "show_extension", extension }),
    hideExtension: (extension: string) =>
      dispatch({ type: "hide_extension", extension }),
  };
}
...
function ExtensionWrapper({
  ...
  useEffect(() => {
    if (
      ...
      (currentConfig.current !== null &&
        !shallowEqual(currentConfig.current, extension.config)) ||
