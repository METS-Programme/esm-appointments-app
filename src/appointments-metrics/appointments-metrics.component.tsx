...
const AppointmentsMetrics: React.FC<AppointmentMetricsProps> = ({
  serviceUuid,
}) => {
  const { t } = useTranslation();

  const { highestServiceLoad, error: clinicalMetricsError } =
    useClinicalMetrics();
  const { totalProviders, isLoading: allAppointmentsLoading } =
    useAllAppointmentsByDate();
  const { totalScheduledAppointments } = useScheduledAppointment(serviceUuid);

  const { currentAppointmentDate } = useAppointmentDate();
  const formattedStartDate = formatDate(parseDate(currentAppointmentDate), {
    mode: "standard",
    time: false,
  });

  // TODO we will need rework these after we discuss the logic we want to use
  const { appointmentList: arrivedAppointments } =
    useAppointmentList("CheckedIn");
  const { appointmentList: pendingAppointments } =
    useAppointmentList("Scheduled");
  ...
  if (clinicalMetricsError) {
    return (
      <ErrorState
        headerTitle={t("appointmentMetricsLoadError", "Metrics load error")}
        error={clinicalMetricsError}
      />
    );
  }
  ...
  return (
    <>
      ...
      <div className={styles.cardContainer} data-testid="clinic-metrics">
        ...
        <MetricsCard
          label={
            highestServiceLoad?.count !== 0
              ? t(highestServiceLoad?.serviceName)
